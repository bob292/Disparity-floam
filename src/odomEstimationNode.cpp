...
pcl::PointCloud<pcl::PointXYZI>::Ptr trajectory_cloud(new pcl::PointCloud<pcl::PointXYZI>());
...
void odom_estimation(){
    while(1){
        if(!pointCloudEdgeBuf.empty() && !pointCloudSurfBuf.empty()){
            ...
            //save to trajectory cloud
            pcl::PointXYZI trajectory_point;
            trajectory_point.x = t_current.x();
            trajectory_point.y = t_current.y();
            trajectory_point.z = t_current.z();
            trajectory_point.intensity = 0.5;
...
int main(int argc, char **argv)
{
    ...
    ros::spin();
    pcl::io::savePCDFileASCII("/floam/trajectory_cloud.pcd",*trajectory_cloud);
    std::cerr<<"Saved "<<trajectory_cloud->size()<<" data trajectory points into file"<<std::endl;
    return 0;
}
